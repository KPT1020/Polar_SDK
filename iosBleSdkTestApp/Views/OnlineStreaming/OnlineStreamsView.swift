...
struct OnlineStreamingButton: View {
    ...
    private func getStreamButtonText(_ feature:PolarDeviceDataType, _ isStreaming: OnlineStreamingState?) -> String {
        ...
        if feature == .ppi {
            ...
            text = getShortNameForDataType(feature)
        }
    ...
    private func streamButtonToggle(_ feature:PolarDeviceDataType) {
        ...
        if(bleSdkManager.isStreamOn(feature: feature)) {
            if (feature == .ppi) {
                bleSdkManager.onlineStreamStop(feature: .ppi)
                bleSdkManager.onlineStreamStop(feature: .hr)
                bleSdkManager.onlineStreamStop(feature: .ppg)
            }else{
            ...
            if(feature == .ppi) {
                bleSdkManager.onlineStreamStart(feature: .ppi)
                bleSdkManager.onlineStreamStart(feature: .hr)
                bleSdkManager.getOnlineStreamSettings(feature: .ppg)
            }else{
